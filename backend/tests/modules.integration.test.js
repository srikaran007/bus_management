...
describe("Module APIs", () => {
  ...
  test("driver endpoints support RBAC + pagination", async () => {
    const { token: adminToken } = await createUserWithToken({
      role: "admin",
      email: "driveradmin@test.com"
    });
    const { token: studentToken } = await createUserWithToken({
      role: "student",
      email: "driverstudent@test.com"
    });
  ...
  test("route endpoints support CRUD + pagination", async () => {
    ...
    const forbiddenDelete = await request(app)
      .delete(`/api/routes/${createRes.body._id}`)
      .set(authHeader(staffToken));
  ...
  test("student endpoints enforce role access", async () => {
    const { user: adminUser, token: adminToken } = await createUserWithToken({
      role: "admin",
      email: "studentadmin@test.com"
    });
    const { user: staffUser, token: staffToken } = await createUserWithToken({
      role: "staff",
      email: "studentstaff@test.com"
    });
    const { user: studentUser, token: studentToken } = await createUserWithToken({
      role: "student",
      email: "std1@test.com"
    });
    const { user: anotherStudentUser } = await createUserWithToken({
      role: "student",
      email: "std2@test.com"
    });
    ...
    const ownRecord = await request(app)
      .get(`/api/students/${studentDoc._id}`)
      .set(authHeader(studentToken));
    ...
    const otherId = allStudents.find((entry) => String(entry.user) === String(anotherStudentUser._id))._id;

    const othersRecord = await request(app).get(`/api/students/${otherId}`).set(authHeader(studentToken));
    expect(othersRecord.statusCode).toBe(403);
  });

  test("attendance and entry-exit enforce driver/staff workflow", async () => {
    const { token: driverToken } = await createUserWithToken({
      role: "driver",
      email: "driveratt@test.com"
    });
    const { token: staffToken } = await createUserWithToken({ role: "staff", email: "staffatt@test.com" });
    const { token: transportToken } = await createUserWithToken({
      role: "transport",
      email: "transportatt@test.com"
    });
